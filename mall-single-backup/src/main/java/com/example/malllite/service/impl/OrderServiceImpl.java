    ...
    public OrderVO createOrder(OrderCreateRequest request) {
        ...
        for (OrderCreateItemRequest itemRequest : request.getItems()) {
            ...
            if (product.getStatus() == null || product.getStatus() == 0) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "商品已下架：" + product.getProductName());
            }

            if (product.getStock() == null || product.getStock() < itemRequest.getQuantity()) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "商品库存不足：" + product.getProductName());
            }
